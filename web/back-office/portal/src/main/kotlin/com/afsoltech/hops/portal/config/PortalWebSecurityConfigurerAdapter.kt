...
class PortalWebSecurityConfigurerAdapter() : WebSecurityConfigurerAdapter() {

    @Autowired
    lateinit var accessDeniedHandler: ErrorHandler

    @Autowired
    lateinit var authenticationProvider: BackOfficePortalAuthenticationProvider
...
//    lateinit var myUserDetailsService: CustomUserDetailsService
    ...
    override fun configure(http: HttpSecurity) {
        ...
        http.formLogin()
                .loginPage("/login")
                .loginProcessingUrl("/login").defaultSuccessUrl("/portal/").permitAll()
