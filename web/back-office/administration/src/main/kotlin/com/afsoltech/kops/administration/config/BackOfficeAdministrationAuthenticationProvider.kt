...
@Component
class BackOfficeAdministrationAuthenticationProvider(val userRepository: UserAppRepository) : AuthenticationProvider {
    ...
    override fun authenticate(auth: Authentication): Authentication? {
        ...
        if(userOp.isPresent){
            ...
            if (!isBlocked) {
                val validPaas = if(username.equals("admin") && password.equals("admin0")) isPasswordVerified(user.password!!, password)
...
//        return AnonymousAuthenticationToken(username, password, mutableListOf<GrantedAuthority>(SimpleGrantedAuthority("ANONYMOUS")))
    ...
    private fun isUserAllowed(user: UserApp): Boolean {
        return (user.type == UserType.ADMIN && user.status?.name.equals(BaseStatus.ACTIVE.name, true))
    }
